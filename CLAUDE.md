...
6. **🔒 사용자 nodeId 절대 준수.** 사용자가 제공한 Figma URL/nodeId 를 오케가 임의로 다른 nodeId 로 교체 금지. 이름이 `Planning` 같이 기획용 뉘앙스여도 그대로 사용 (실제 디자인 페이지일 가능성 높음). 자체 페이지 탐색 금지. 빈 페이지/404 등 **명백한 구조 이상 시에만** 사용자에게 재확인 질문 (상세: `docs/workflow.md` Phase 2)
