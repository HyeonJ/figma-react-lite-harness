...
# 있으면 토큰 추출이 그 페이지만 스캔 + 레이어명 기반 네이밍 → 품질 향상.
...
- extract-tokens.sh 자동 실행 → src/styles/tokens.css 생성
  - Component URL 제공됐으면 --component-url 로 전달
...
> **Component 페이지 판별 팁**: Figma 파일 왼쪽 사이드바 페이지 목록에서 "Components" / "Design System" / "Tokens" / "DS" / "UI Kit" 같은 이름의 페이지가 있으면 거기가 맞을 확률 높음. 없으면 생략해도 됨 (전체 파일 스캔 fallback).
...
2. scripts/extract-tokens.sh 재실행 (docs/project-context.md 기반 모드 자동 판정):
   - Component Page Node ID 필드에 값 있으면:
       bash scripts/extract-tokens.sh <fileKey> --component-page <nodeId>
   - 비어있으면:
